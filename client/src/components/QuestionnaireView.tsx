    ...
    <div className="grid">
      <div className="card">
        ...
        <div className="grid-2">
          <input placeholder={locale === 'en' ? 'Name' : '氏名'} value={profile.name} onChange={(e) => setProfile({ ...profile, name: e.target.value })} />
          <input placeholder={locale === 'en' ? 'Company' : '会社名'} value={profile.company} onChange={(e) => setProfile({ ...profile, company: e.target.value })} />
          <input placeholder={locale === 'en' ? 'Title' : '役職'} value={profile.title} onChange={(e) => setProfile({ ...profile, title: e.target.value })} />
          <input placeholder="Email" value={profile.email} onChange={(e) => setProfile({ ...profile, email: e.target.value })} />
        </div>
        <p className="small" style={{ marginTop: 12 }}>{completed} / {questions.length}</p>
      </div>

      {questions.map((q, index) => (
        <div className="card" key={q.id}>
          <div className="small">{index + 1}. {q.category}</div>
      ...
      <div className="card">
        <button className="primary" disabled={completed !== questions.length || loading} onClick={submit}>
      ...
      {summary && (
        <div className="grid-2">
          <div className="card">
            ...
            <p><strong>{text.interpretation}:</strong> {summary.orientation[locale]}</p>
            <div className="grid">
          ...
          <div className="card">
            <h2>{text.similar}</h2>
            <div className="grid">
              {similar.length === 0 ? <div className="small">No matches yet.</div> : similar.map((match) => (
